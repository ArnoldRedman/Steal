...
/// <typeparam name="T"></typeparam>
    ...
    {   
        ...
        if (CurrentShowPanel == null)
        {
            ...
            CurrentShowPanel.GetComponent<RectTransform>().sizeDelta = new Vector2(1920, 1080);//调整UI宽度和高度
        ...
        if (UIPanelDict.ContainsKey(panelName))
        {
            ...
            UIPanelDict[panelName].transform.SetParent(CurrentShowPanel.transform);//打开的时候要设置面板的父亲为CurrentShowPanel
        ...
        else//不在字典中 创建一个新的面板 存到我们的字典中 
        {
            ...
            Vector3 achor = panelObj.GetComponent<RectTransform>().anchoredPosition3D;
            ...
            panelObj.GetComponent<RectTransform>().anchoredPosition3D=achor;
...
/// <typeparam name="T"></typeparam>
    public void closePanel<T>()
    {
        ...
        {
            UIPanelDict[typeof(T).Name].Hide();  //关闭的时候不需要销毁  
            // 设置面板的父亲为canvas   这时候又放到  CurrentShowPanel的外面    
            // UIPanelDict[typeof(T).Name].transform.SetParent(Canvas.transform);
